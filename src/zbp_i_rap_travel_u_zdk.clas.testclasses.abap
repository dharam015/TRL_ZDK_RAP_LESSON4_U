...
*  ENDMETHOD.
  ...
  METHOD create_travel.
    ...
    DATA travels_in TYPE TABLE FOR CREATE zi_rap_travel_U_zdk\\travel.

    travels_in = VALUE #( ( agencyid     = 1        "Agency 070001 does exist, Agency 1 does not exist
                            customerid   = 1
                            begindate    = today
                            enddate      = today + 30
                            bookingfee   = 30
    ...
    MODIFY ENTITIES OF zi_rap_travel_u_zdk
      ENTITY travel
    ...
    SELECT * FROM zi_rap_travel_u_zdk WHERE TravelId = @new_travel_id INTO TABLE @DATA(lt_travel)  .
  ...
  METHOD class_setup.
    cds_test_environment = cl_cds_test_environment=>create_for_multiple_cds(
      i_for_entities = VALUE #( ( i_for_entity = 'zi_rap_travel_u_zdk' )
                                ( i_for_entity = 'zi_rap_booking_u_zdk' ) )
    ).
